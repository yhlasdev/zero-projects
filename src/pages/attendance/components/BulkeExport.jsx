...
export default function BulkExportModal({ open, onClose, rows = [], date }) {
  ...
  const handleExport = () => {
    ...
    const runExport = () => {
      ...
      doc.setFontSize(14);
      doc.setFont("helvetica", "bold");
      doc.text(t("attendance.pdfReport"), pageW / 2, 10, { align: "center" });
      doc.setFontSize(8);
      doc.setFont("helvetica", "normal");
      doc.text(t("attendance.pdfDescription"), pageW / 2, 16, {
        align: "center",
      });
      doc.setFontSize(7);
      doc.setTextColor(160, 200, 255);
      doc.text(t("attendance.period"), pageW - margin, 7, { align: "right" });
      doc.setTextColor(255, 255, 255);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      ...
      const statItems = [
        { label: t("attendance.totalEmp"), value: s(selectedRows.length) },
        { label: t("attendance.present"), value: s(stats.present), color: [34, 140, 80] },
        { label: t("attendance.late"), value: s(stats.late), color: [202, 138, 4] },
        { label: t("attendance.absent"), value: s(stats.absent), color: [220, 38, 38] },
        { label: t("attendance.totalHours"), value: stats.totalH.toFixed(2) + "h" },
        { label: t("attendance.avgHours"), value: stats.avgH.toFixed(2) + "h" },
      ];
      ...
      doc.text(t("attendance.pdfEmployeeDetails"), margin, sY + 24);
      const tY = sY + 28;
      const cols = [
        { label: t("common.id"), w: 18 },
        { label: t("common.employee"), w: 35 },
        { label: t("common.department"), w: 28 },
        { label: t("common.position"), w: 35 },
        { label: t("common.checkIn"), w: 18 },
        { label: t("common.checkOut"), w: 18 },
        { label: t("dashboard.hours"), w: 14 },
      ...
      selectedRows.forEach((item) => {
        ...
        const vals = [
          ...
          s(
            item.status
              ? t(`attendance.${(item.status || "").toLowerCase()}`)
              : "-",
          ),
        ];
        cx = margin;
      ...
      doc.text(t("attendance.pdfConfidential"), pageW / 2, footY + 4, {
        align: "center",
      });
      doc.text(t("attendance.pdfPage", { page: 1, total: 1 }), pageW - margin, footY + 4, { align: "right" });
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ p: 0, bgcolor: "#f8fafc" }}>
        ...
        >
          {[
            { label: t("attendance.totalEmp"), value: rows.length },
            { label: t("attendance.present"), value: stats.present, color: "#22c55e" },
            { label: t("attendance.late"), value: stats.late, color: "#f59e0b" },
            { label: t("attendance.absent"), value: stats.absent, color: "#ef4444" },
            { label: t("attendance.totalHours"), value: stats.totalH.toFixed(2) + "h" },
            { label: t("attendance.avgHours"), value: stats.avgH.toFixed(2) + "h" },
          ].map((st, i) => (
            <Box
        ...
        >
          <Typography sx={{ fontSize: 12, color: "#64748b", mr: 1 }}>
            {t("common.showing")} {selected.size} {t("common.of")} {rows.length} {t("common.selected")}
          </Typography>
          <Checkbox
            size="small"
            checked={allSelected}
        ...
        >
          ...
          >
            {t("attendance.exportRecords", { count: selected.size })}
          </Button>
        </Box>
      </DialogContent>
    </Dialog>
